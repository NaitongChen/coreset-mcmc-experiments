...
Threads.@threads for i in 1:n_run
    ...
    KLs[i,5] = load("linear_regression_austerity_" * "0.01_" * string(i) * ".jld", "kl")
...
################################
################################
D_stan = load("../stan_results/stan_lin_reg.jld")["data"]
...
Threads.@threads for i in 1:n_run
    ...
    m_method = load("linear_regression_SHF_" * string(500) * "_" * string(i) * ".jld", "mean")
    mrels[i,3] = norm(m_method - stan_mean) / norm(stan_mean)
    m_method = load("linear_regression_uniform_" * string(500) * "_" * string(i) * ".jld", "mean")
    mrels[i,4] = norm(m_method - stan_mean) / norm(stan_mean)
    m_method = load("linear_regression_austerity_" * "0.01_" * string(i) * ".jld", "mean")
    mrels[i,5] = norm(m_method - stan_mean) / norm(stan_mean)
    m_method = load("linear_regression_confidence_" * "0.01_" * string(i) * ".jld", "mean")
    mrels[i,6] = norm(m_method - stan_mean) / norm(stan_mean)
    m_method = load("linear_regression_sgld_" * string(i) * ".jld", "mean")
...
Threads.@threads for i in 1:n_run
    ...
    m_method = load("linear_regression_SHF_" * string(500) * "_" * string(i) * ".jld", "cov")
    srels[i,3] = norm(m_method - stan_cov) / norm(stan_cov)
    m_method = load("linear_regression_uniform_" * string(500) * "_" * string(i) * ".jld", "cov")
    srels[i,4] = norm(m_method - stan_cov) / norm(stan_cov)
    m_method = load("linear_regression_austerity_" * "0.01_" * string(i) * ".jld", "cov")
    srels[i,5] = norm(m_method - stan_cov) / norm(stan_cov)
    m_method = load("linear_regression_confidence_" * "0.01_" * string(i) * ".jld", "cov")
    srels[i,6] = norm(m_method - stan_cov) / norm(stan_cov)
    m_method = load("linear_regression_sgld_" * string(i) * ".jld", "cov")
...
for i in 1:n_run
    ...
    m_method = load("linear_regression_QNC_" * string(500) * "_" * string(i) * ".jld", "θs")
    ...
    m_method = load("linear_regression_SHF_" * string(500) * "_" * string(i) * ".jld", "θs")
    ...
    m_method = load("linear_regression_uniform_" * string(500) * "_" * string(i) * ".jld", "θs")
    time = load("linear_regression_uniform_" * string(500) * "_" * string(i) * ".jld", "c_time")
    ...
    m_method = load("linear_regression_austerity_" * "0.01_" * string(i) * ".jld", "θs")
    time = load("linear_regression_austerity_" * "0.01_" *string(i) * ".jld", "c_time")
    time = time[end] - time[10001]
    m_method = reduce(hcat, m_method)'
    m_method = m_method[10001:end, :]
    split = rand(10000, 1, size(D_stan, 2))
    ...
    m_method = load("linear_regression_confidence_" * "0.01_" * string(i) * ".jld", "θs")
    time = load("linear_regression_confidence_" * "0.01_" * string(i) * ".jld", "c_time")
    time = time[end] - time[10001]
    m_method = reduce(hcat, m_method)'
    m_method = m_method[10001:end, :]
    split = rand(10000, 1, size(D_stan, 2))
    ...
    m_method = load("linear_regression_sgld_" * string(i) * ".jld", "θs")
    time = load("linear_regression_sgld_" * string(i) * ".jld", "c_time")
    time = time[end] - time[10001]
    m_method = reduce(hcat, m_method)'
    m_method = m_method[10001:end, :]
    split = rand(10000, 1, size(D_stan, 2))
    ...
    m_method = load("linear_regression_sghmc_" * string(i) * ".jld", "θs")
    time = load("linear_regression_sghmc_" * string(i) * ".jld", "c_time")
    time = time[end] - time[10001]
    m_method = reduce(hcat, m_method)'
    m_method = m_method[10001:end, :]
    split = rand(10000, 1, size(D_stan, 2))
