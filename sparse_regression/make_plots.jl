...
Threads.@threads for i in 1:n_run
    println(i)
    KLs[i,1] = load("sparse_regression_coresetMCMC_" * "0.5_0.5_" * string(500) * "_" * string(i) * ".jld", "kl")
...
Threads.@threads for i in 1:n_run
    println(i)
    m_method = load("sparse_regression_coresetMCMC_" * "0.5_0.5_" * string(500) * "_" * string(i) * ".jld", "mean")
...
Threads.@threads for i in 1:n_run
    println(i)
    m_method = load("sparse_regression_coresetMCMC_" * "0.5_0.5_" * string(500) * "_" * string(i) * ".jld", "cov")
...
Threads.@threads for i in 1:n_run
    println(i)
    println("cmcmcs")
    gamma_m = reduce(hcat, load("sparse_regression_coresetMCMC_" * "0.5_0.5_" * string(500) * "_" * string(i) * ".jld", "θs"))'[end-n_samples+1:end,6:10]
    gamma_m = [gamma_m[i,:] for i in 1:size(gamma_m,1)]
    mapm = countmap(gamma_m)
    KLDs[i,1] = 0
    for (key, value) in mapstan
        if haskey(mapm, key)
            KLDs[i,1] += mapstan[key]/100000 * (log(mapstan[key]/100000) - log(mapm[key]/10000))
        end
    end

    println("qnc")
    gamma_m = reduce(hcat, load("sparse_regression_QNC_" * string(500) * "_" * string(i) * ".jld", "θs"))'[end-n_samples+1:end,6:10]
    gamma_m = [gamma_m[i,:] for i in 1:size(gamma_m,1)]
    mapm = countmap(gamma_m)
    KLDs[i,2] = 0
    for (key, value) in mapstan
        if haskey(mapm, key)
            KLDs[i,2] += mapstan[key]/100000 * (log(mapstan[key]/100000) - log(mapm[key]/10000))
        end
    end

    println("unif")
    gamma_m = reduce(hcat, load("sparse_regression_uniform_" * string(500) * "_" * string(i) * ".jld", "θs"))'[end-n_samples+1:end,6:10]
    gamma_m = [gamma_m[i,:] for i in 1:size(gamma_m,1)]
    mapm = countmap(gamma_m)
    KLDs[i,3] = 0
    for (key, value) in mapstan
        if haskey(mapm, key)
            KLDs[i,3] += mapstan[key]/100000 * (log(mapstan[key]/100000) - log(mapm[key]/10000))
        end
    end

    println("austerity")
    gamma_m = reduce(hcat, load("sparse_regression_austerity_" * "0.001_0.5_" * string(i) * ".jld", "θs"))'[end-n_samples+1:end,6:10]
    gamma_m = [gamma_m[i,:] for i in 1:size(gamma_m,1)]
    mapm = countmap(gamma_m)
    KLDs[i,4] = 0
    for (key, value) in mapstan
        if haskey(mapm, key)
            KLDs[i,4] += mapstan[key]/100000 * (log(mapstan[key]/100000) - log(mapm[key]/10000))
        end
    end

    println("confidence")
    gamma_m = reduce(hcat, load("sparse_regression_confidence_" * "0.001_0.5_" * string(i) * ".jld", "θs"))'[end-n_samples+1:end,6:10]
    gamma_m = [gamma_m[i,:] for i in 1:size(gamma_m,1)]
    mapm = countmap(gamma_m)
    KLDs[i,5] = 0
    for (key, value) in mapstan
        if haskey(mapm, key)
            KLDs[i,5] += mapstan[key]/100000 * (log(mapstan[key]/100000) - log(mapm[key]/10000))
        end
    end

    println("cmcmc")
    gamma_m = reduce(hcat, load("sparse_regression_coresetMCMC_" * "0.01_" * string(500) * "_" * string(i) * ".jld", "θs"))'[end-n_samples+1:end,6:10]
    gamma_m = [gamma_m[i,:] for i in 1:size(gamma_m,1)]
    mapm = countmap(gamma_m)
    KLDs[i,6] = 0
    for (key, value) in mapstan
        if haskey(mapm, key)
            KLDs[i,6] += mapstan[key]/100000 * (log(mapstan[key]/100000) - log(mapm[key]/10000))
        end
    end
end
...
for i in 1:n_run
    println(i)
    println("cmcmcs")
    m_method = load("sparse_regression_coresetMCMC_" * "0.5_0.5_" * string(500) * "_" * string(i) * ".jld", "θs")
    time = load("sparse_regression_coresetMCMC_" * "0.5_0.5_" * string(500) * "_" * string(i) * ".jld", "c_time")
