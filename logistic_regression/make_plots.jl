...
Threads.@threads for i in 1:n_run
    println(i)
    KLs[i,1] = load("logistic_regression_coresetMCMC_" * "0.5_" * string(500) * "_" * string(i) * ".jld", "kl")
    if isfile("logistic_regression_QNC_" * string(500) * "_" * string(i) * ".jld")
        KLs[i,2] = load("logistic_regression_QNC_" * string(500) * "_" * string(i) * ".jld", "kl")
    else
        KLs[i,2] = NaN
    end
    KLs[i,3] = load("logistic_regression_SHF_" * string(500) * "_" * string(i) * ".jld", "kl")
    KLs[i,4] = load("logistic_regression_uniform_" * string(500) * "_" * string(i) * ".jld", "kl")
    KLs[i,5] = load("logistic_regression_austerity_" * "0.001_" * string(i) * ".jld", "kl")
    KLs[i,6] = load("logistic_regression_confidence_" * "0.001_" * string(i) * ".jld", "kl")
    KLs[i,7] = load("logistic_regression_sgld_" * string(i) * ".jld", "kl")
    KLs[i,8] = load("logistic_regression_sghmc_" * string(i) * ".jld", "kl")
    KLs[i,9] = load("logistic_regression_coresetMCMC_" * "2_" * string(500) * "_" * string(i) * ".jld", "kl")
end
...
Threads.@threads for i in 1:n_run
    println(i)
    m1 = load("logistic_regression_coresetMCMC_" * "0.5_" *string(500) * "_" * string(i) * ".jld", "mean")
    mrels[i,1] = norm(m1 - stan_mean) / norm(stan_mean)
    if isfile("logistic_regression_QNC_" * string(500) * "_" * string(i) * ".jld")
        m_method = load("logistic_regression_QNC_" * string(500) * "_" * string(i) * ".jld", "mean")
        mrels[i,2] = norm(m_method - stan_mean) / norm(stan_mean)
    else
        mrels[i,2] = NaN
    end
    m_method = load("logistic_regression_SHF_" * string(500) * "_" * string(i) * ".jld", "mean")
    mrels[i,3] = norm(m_method - stan_mean) / norm(stan_mean)
    m_method = load("logistic_regression_uniform_" * string(500) * "_" * string(i) * ".jld", "mean")
    mrels[i,4] = norm(m_method - stan_mean) / norm(stan_mean)
    m_method = load("logistic_regression_austerity_" * "0.001_" * string(i) * ".jld", "mean")
    mrels[i,5] = norm(m_method - stan_mean) / norm(stan_mean)
    m_method = load("logistic_regression_confidence_" * "0.001_" * string(i) * ".jld", "mean")
    mrels[i,6] = norm(m_method - stan_mean) / norm(stan_mean)
    m_method = load("logistic_regression_sgld_" * string(i) * ".jld", "mean")
    mrels[i,7] = norm(m_method - stan_mean) / norm(stan_mean)
    m_method = load("logistic_regression_sghmc_" * string(i) * ".jld", "mean")
    mrels[i,8] = norm(m_method - stan_mean) / norm(stan_mean)
    m_method = load("logistic_regression_coresetMCMC_" * "2_" *string(500) * "_" * string(i) * ".jld", "mean")
...
Threads.@threads for i in 1:n_run
    println(i)
    m1 = load("logistic_regression_coresetMCMC_" * "0.5_" * string(500) * "_" * string(i) * ".jld", "cov")
    srels[i,1] = norm(m1 - stan_cov) / norm(stan_cov)
    if isfile("logistic_regression_QNC_" * string(500) * "_" * string(i) * ".jld")
        m_method = load("logistic_regression_QNC_" * string(500) * "_" * string(i) * ".jld", "cov")
        srels[i,2] = norm(m_method - stan_cov) / norm(stan_cov)
    else
        srels[i,2] = NaN
    end
    m_method = load("logistic_regression_SHF_" * string(500) * "_" * string(i) * ".jld", "cov")
    srels[i,3] = norm(m_method - stan_cov) / norm(stan_cov)
    m_method = load("logistic_regression_uniform_" * string(500) * "_" * string(i) * ".jld", "cov")
    srels[i,4] = norm(m_method - stan_cov) / norm(stan_cov)
    m_method = load("logistic_regression_austerity_" * "0.001_" * string(i) * ".jld", "cov")
    srels[i,5] = norm(m_method - stan_cov) / norm(stan_cov)
    m_method = load("logistic_regression_confidence_" * "0.001_" * string(i) * ".jld", "cov")
    srels[i,6] = norm(m_method - stan_cov) / norm(stan_cov)
    m_method = load("logistic_regression_sgld_" * string(i) * ".jld", "cov")
    srels[i,7] = norm(m_method - stan_cov) / norm(stan_cov)
    m_method = load("logistic_regression_sghmc_" * string(i) * ".jld", "cov")
    srels[i,8] = norm(m_method - stan_cov) / norm(stan_cov)
    m_method = load("logistic_regression_coresetMCMC_" * "2_" * string(500) * "_" * string(i) * ".jld", "cov")
...
for i in 1:n_run
    println(i)
    m_method = load("logistic_regression_coresetMCMC_" * "0.5_" * string(500) * "_" * string(i) * ".jld", "θs")
    time = load("logistic_regression_coresetMCMC_" * "0.5_" * string(500) * "_" * string(i) * ".jld", "c_time")
    ...
    if isfile("logistic_regression_QNC_" * string(500) * "_" * string(i) * ".jld")
        m_method = load("logistic_regression_QNC_" * string(500) * "_" * string(i) * ".jld", "θs")
        time = load("logistic_regression_QNC_" * string(500) * "_" * string(i) * ".jld", "c_time")
    ...
    m_method = load("logistic_regression_SHF_" * string(500) * "_" * string(i) * ".jld", "θs")
    time = load("logistic_regression_SHF_" * string(500) * "_" * string(i) * ".jld", "c_time")
    ...
    m_method = load("logistic_regression_uniform_" * string(500) * "_" * string(i) * ".jld", "θs")
    time = load("logistic_regression_uniform_" * string(500) * "_" * string(i) * ".jld", "c_time")
    ...
    m_method = load("logistic_regression_austerity_" * "0.001_" * string(i) * ".jld", "θs")
    time = load("logistic_regression_austerity_" * "0.001_" *string(i) * ".jld", "c_time")
    time = time[end] - time[10001]
    m_method = reduce(hcat, m_method)'
    m_method = m_method[10001:end, :]
    split = rand(10000, 1, size(D_stan, 2))
    ...
    m_method = load("logistic_regression_confidence_" * "0.001_" * string(i) * ".jld", "θs")
    time = load("logistic_regression_confidence_" * "0.001_" * string(i) * ".jld", "c_time")
    time = time[end] - time[10001]
    m_method = reduce(hcat, m_method)'
    m_method = m_method[10001:end, :]
    split = rand(10000, 1, size(D_stan, 2))
    ...
    m_method = load("logistic_regression_sgld_" * string(i) * ".jld", "θs")
    time = load("logistic_regression_sgld_" * string(i) * ".jld", "c_time")
    time = time[end] - time[10001]
    m_method = reduce(hcat, m_method)'
    m_method = m_method[10001:end, :]
    split = rand(10000, 1, size(D_stan, 2))
    ...
    m_method = load("logistic_regression_sghmc_" * string(i) * ".jld", "θs")
    time = load("logistic_regression_sghmc_" * string(i) * ".jld", "c_time")
    time = time[end] - time[10001]
    m_method = reduce(hcat, m_method)'
    m_method = m_method[10001:end, :]
    split = rand(10000, 1, size(D_stan, 2))
    ...
    m_method = load("logistic_regression_coresetMCMC_" * "2_" * string(500) * "_" * string(i) * ".jld", "θs")
    time = load("logistic_regression_coresetMCMC_" * "2_" * string(500) * "_" * string(i) * ".jld", "c_time")
    time = time[end] - time[25001]
    m_method = reduce(hcat, m_method)'
    m_method = m_method[50001:end, :]
    split = rand(10000, 1, size(D_stan, 2))
...
boxplot!([names[3]], trains[:,3], label = names[3], color = colours[3], legend=false)
